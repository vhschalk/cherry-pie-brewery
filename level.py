...
def reading(sensor):
    ...
    GPIO.setup(echo_in_pin,GPIO.OUT)
    GPIO.setup(trig_out_pin,GPIO.IN)
    GPIO.output(echo_in_pin, GPIO.LOW)
    ...
    # change this value to the pin you are using
    # GPIO input = the pin that's connected to "Echo" on the sensor
    while GPIO.input(trig_out_pin) == 0:
      signaloff = time.time()
